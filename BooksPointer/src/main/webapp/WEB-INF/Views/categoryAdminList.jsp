		...
		<c:forEach var="p" varStatus="st" items="${cartInfo }">
			<tr>
				...
				<td class="span2"><c:set var="contextRoot"
						...
						<a 	class="btn btn-danger" role="button" href="<c:url value="/deleteProd/${p.cartId }"/>">Delete</a></td>
			</tr>
		</c:forEach>
		<tr>
		<td><c:set var="gtot" value="${gtot+p.price*p.qty }"></c:set> <span
			class="col-lg-9" align="right"><label>Grand Total</label>
			<c:out value="${gtot }" /></span></td>
<!-- 
		<tfoot>
		<td>
